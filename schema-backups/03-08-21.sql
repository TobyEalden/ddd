...
ALTER TABLE ONLY public.actor_key DROP CONSTRAINT entitykey_user_id_fkey;
ALTER TABLE ONLY public.actor DROP CONSTRAINT actor_user_fk;
...
--
-- Name: actor actor_user_fk; Type: FK CONSTRAINT; Schema: public; Owner: supabase_admin
--

ALTER TABLE ONLY public.actor
    ADD CONSTRAINT actor_user_fk FOREIGN KEY (user_id) REFERENCES auth.users(id);


--
-- Name: actor_key entitykey_user_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: supabase_admin
--

ALTER TABLE ONLY public.actor_key
    ADD CONSTRAINT entitykey_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id);
...
CREATE POLICY "Enable users to add their own keys" ON public.actor_key FOR INSERT WITH CHECK ((auth.uid() = user_id));
...
CREATE POLICY "Enable users to update their own keys" ON public.actor_key FOR UPDATE USING ((auth.uid() = user_id)) WITH CHECK ((auth.uid() = user_id));
...
CREATE POLICY "User key access" ON public.actor_key FOR SELECT USING ((auth.uid() = user_id));
